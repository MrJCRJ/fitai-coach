...
- Geração de treinos personalizados por IA
...
- Opção por Assinatura (ECONÔMICO): R$ 15,00/mês = Acesso ilimitado.
  ...
  2.2 Projeções Financeiras (Ano 1)

| Trimestre | Usuários Ativos | Receita Mensal | Lucro Mensal |
| --------- | --------------: | -------------: | -----------: |
| 1º        |             500 |       R$ 6.480 |     R$ 5.180 |
| 2º        |           1.000 |      R$ 12.960 |    R$ 11.160 |
| 3º        |           2.000 |      R$ 25.920 |    R$ 23.320 |
| 4º        |           3.500 |      R$ 45.360 |    R$ 41.560 |

2.3 Estrutura de Custos

| Item            |  Custo Mensal | Observações            |
| --------------- | ------------: | ---------------------- |
| DeepSeek API    |  R$ 50-R$ 200 | Escalável com uso      |
| Marketing       | R$ 200-R$ 500 | Performance-based      |
| Infraestrutura  |   R$ 0-R$ 100 | Vercel + serviços free |
...
IA: DeepSeek API (modelo: deepseek-chat). Custo aproximado R$ 0,02 por requisição (verificar).
...
4.1 Primeiro Acesso: Boas-vindas → Avaliação Inicial → IA → Primeiro Treino → Escolha do Acesso (Anúncios vs Assinatura).
...
- Sprint 1: Setup Next.js + PWA; Identificação do dispositivo; Avaliação física básica; Integração DeepSeek.
- Sprint 2: Core: Geração de treinos por IA; Sistema de anúncios rewarded; Armazenamento local seguro; Dashboard.
- Sprint 3: Monetização: horas premium; gateway de pagamento; upsell.
- Sprint 4: Polimento: anti-fraude, performance, testes.

  6.2 MVP: Avaliação física 5 testes, 1 treino personalizado, anúncio rewarded, assinatura simples, progresso básico.

---

## 7. Análise de Riscos

7.1 Técnicos: DeepSeek instável (cache/fallback), LocalStorage limitado (backup), Fraude (validações), PWA performance (lazy load).
...
Currently `src/lib/ai/deepseek.ts` is a lightweight stub that returns a mocked workout plan. Replace the stub with a real DeepSeek API integration and keep keys in `.env`.
